...
%     plot3(pxs, pys, pbs, 'g')
    ...
    [tzz windows] = drawWindows(xover, txx, tyy, tzz);
...
function [tzz, win] = drawWindows(xover, txx, tyy, tzz)
    global upxs
    xf = find(xover > 850 & xover < 912);
    from = xf(1);
    to = xf(end);
%     for ndx = 1:length(xf)
%         pt = xf(ndx);
...
%     end
    ...
    P7.x = txx(nx7,1) + 2;
    P7.y = tyy(nx7,nxy7);
    yps = tyy(nx7,nxy7-1:nxy7+1);
    zps = tzz(nx7,nxy7-1:nxy7+1);
    P7.z = interp1(yps, zps, P7.y);
    plotPts([P1 P2 P3 P3A P4 P5 P6 P7 P1],'g*-');
    P4A = P4;
    P4A.x = P4A.x+0.1;
    P5A = P5;
    P5A.x = P5A.x+0.1;
    w1 = mkWin([P5 P6 P7], [P4 P3], 50, 60);
    w2 = mkWin([P7 P1], [P3 P2], 50, 40);
    [r,c] = size(w1.xx);
    pr = r*c;
    for wn = 1:2
        for ndx = 1:length(xf)
            ...
            for ypnx = 1:length(yplus)
                ...
                if ~(offTop && isnan(zv))
                    fprintf('%s consider [%0.1f, %0.1f, %0.1f] -- ', ...
                            str, x, y, tzz(pt, yp))
                end
                ...
                if offTop && ~isnan(zv)
                    ...
                    tyy(pt, yp) = interp1(win.topX, win.topY, x);
                    tzz(pt, yp) = interp1(win.topX, win.topZ, x);
                ...
                elseif ~offTop && isnan(zv)
                    ...
                    tyy(pt, yp) = interp1(win.botX, win.botY, x);
                    tzz(pt, yp) = interp1(win.botX, win.botZ, x);
                    tyy(pt, 65 - yp) = -tyy(pt, yp); 
                    tzz(pt, 65 - yp) = tzz(pt, yp); 
                    fprintf(' moved point to [%0.1f, %0.1f, %0.1f\n', ...
                        x, tyy(pt, yp), tzz(pt, yp))
...
function showWhite(tr)
    [rows cols] = size(tr.xx);
    surf(tr.xx, tr.yy, tr.zz, ones(rows,cols,3))
    hold on
end
